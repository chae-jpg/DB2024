...
public class ProfessorDAO {
    ...
    public List<Professor> searchProfessors(String searchType, String searchText) {
        ...
        switch (searchType) {//텍스트필드의 값의 속성에따라 달라지는 쿼리
            case "교수명":
                query = "SELECT * FROM db2024_StudentViewProfessor WHERE Name LIKE ?";//텍스트 필드의 정보가 이름일때
                break;
            case "교수id":
                query = "SELECT * FROM db2024_StudentViewProfessor WHERE ProfessorID = ?";//텍스트 필드의 정보가 교수id일때
                break;
            case "학과":
                query = "SELECT * FROM db2024_StudentViewProfessor WHERE Department LIKE ?";//텍스트 필드의 정보가 학과일때
                break;
        }
